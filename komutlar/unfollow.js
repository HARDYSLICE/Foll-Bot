...
exports.run = async (client, message, args) => {
  ...
  var unfollowuser = message.mentions.users.first()
  
  if (!db.has(`profile_${message.guild.id}_${message.mentions.users.first().id}_followersme_${message.author.id}`)) {
    
    const embed = new Discord.MessageEmbed()
    .setColor(ayarlar.embedColor)
    .setDescription(`You are not following anyway.`)
    return message.channel.send(embed)
    
  }
  
  db.add(`profile_${message.guild.id}_${message.mentions.users.first().id}_followers`, -1)
  db.add(`profile_${message.guild.id}_${message.author.id}_following`, -1)
  db.delete(`profile_${message.guild.id}_${message.mentions.users.first().id}_followersme_${message.author.id}`)
  
  var xx = db.get(`profile_${message.guild.id}_${unfollowuser.id}_notif`) || "off"
  
  if (xx == "on") {
    
    unfollowuser.send(`<@${message.author.id}> unfollowed you.`)
    
  }
  
  const embed = new Discord.MessageEmbed()
  .setColor(ayarlar.embedColor)
  .setDescription(`You have successfully unfollowed.`)
